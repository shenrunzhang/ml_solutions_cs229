...
def choose_action(state, mdp_data):
    ...
    t_probs = mdp_data['transition_probs']

    # select state-action probabilities starting at state
    P_sa = t_probs[state]

    # add dimension and transpose value function to do elementwise multiplication
    V = np.expand_dims(value, axis=0).T
    
    # perform summation along columns (end up with 1 by 2 array)
    sigma = (P_sa * V).sum(axis=0)


    if np.all(sigma == sigma[0]):
        # return random action if entries of sigma are same
        return np.random.randint(0,2)
    else:
        # else return action corresponding to highest sigma entry
        return sigma.argmax(axis=0)
...
def update_mdp_transition_probs_reward(mdp_data):
    ...
    t_counts = mdp_data['transition_counts']
    n_states = mdp_data['num_states']

    # update rewards
    reward = - reward_counts[:,0] / reward_counts[:,1]
    mdp_data['reward'] = np.where(np.isnan(reward), 0., reward)

    # calculate new transition probability
    t_prob = t_counts / t_counts.sum(axis=1)[:,None]
    mdp_data['transition_probs'] = np.where(np.isnan(t_prob), 1 / n_states, t_prob)
...
def update_mdp_value(mdp_data, tolerance, gamma):
    ...
    # set all values to 0
    #mdp_data['value'] = np.zeros_like(mdp_data['value'])
    ...
    while True: 
        # calculate new values for all states at once
        V = np.expand_dims(value, axis=0).T
        new_value = rewards + gamma * np.max((probs * V).sum(axis=1), axis=1)

        iter_count += 1
        #if any change in value is lower than tolerance, then break
        if np.any(np.abs(new_value - value) < tolerance * 0.001):
...
def test_model(mdp_data, save_path):
    '''
    Runs the model after training is completed and visualize the trial
    '''
    NUM_STATES = 163
    pause_time = 0.0001
    ...
    while state != NUM_STATES - 1:
        ...
        frame = cart_pole.save_frame(state_tuple, pause_time, num)
    ...
    # save to gif
    img_list[0].save(save_path, save_all=True, append_images=img_list[1:], loop=0, duration=100)  # Adjust duration as needed
...
def main(plot=True):
    ...
    test_model(mdp_data, "problemset_solutions/code/cartpole_qlearning/result.gif")
